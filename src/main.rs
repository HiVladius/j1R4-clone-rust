...
async fn main(#[shuttle_runtime::Secrets] secrets: SecretStore) -> shuttle_axum::ShuttleAxum {
    ...
    let config =
        Arc::new(Config::from_secrets(&secrets).expect("Error al cargar la configuración"));

    let db = DatabaseState::init(&config.database_url, &config.database_name)
        .await
        .map_err(|e| {
            shuttle_runtime::Error::Custom(anyhow::Error::msg(format!("Database error: {}", e)))
        })?;
