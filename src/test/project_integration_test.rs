...
async fn test_example(){
    ...
    let response = app
        ...
        .await
        .unwrap();
    assert_eq!(response.status(), axum::http::StatusCode::OK, "Expected status code 200 OK");
}
...
async fn test_project_creation() {
    ...
    if register_status != axum::http::StatusCode::CREATED {
        let body_bytes = axum::body::to_bytes(register_response.into_body(), usize::MAX).await.unwrap();
        let error_msg = String::from_utf8_lossy(&body_bytes);
        panic!("Register failed with status: {}. Error: {}", register_status, error_msg);
    }
    ...
    assert_eq!(response.status(), axum::http::StatusCode::CREATED, "Expected status code 201 CREATED");
}