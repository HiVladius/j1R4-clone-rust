...
pub async fn get_auth_token(app: &Router, username: &str, email: &str) -> String {
    ...
    if !register_status.is_success() {
        panic!(
            "Registration failed with status {} and body: {}",
            register_status, register_body_str
        );
    }
...
pub async fn get_auth_token_and_id(
    app: &Router,
    username: &str,
    email: &str,
) -> (String, ObjectId) {
...
pub async fn create_project_for_user(app: &Router, token: &str, key: &str) -> String {
    ...
    assert_eq!(
        response.status(),
        StatusCode::CREATED,
        "Helper 'create_project_for_user' falló"
    );
    let project: Project =
...
pub async fn add_member_to_project(
    ...
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "Helper 'add_member_to_project' falló"
    );
}
...
pub async fn create_task_for_project(
    ...
    assert_eq!(
        response.status(),
        StatusCode::CREATED,
        "Helper 'create_task_for_project' falló"
    );
    let task: Task =
        serde_json::from_slice(&to_bytes(response.into_body(), usize::MAX).await.unwrap()).unwrap();
